...
void Main()
{
	...
	foreach (string line in Regex.Split(info, Environment.NewLine))
	{
		...
		string desc = string.Join(" ", parts.Skip(index+1));
		//foreach (string p in parts) { p.Dump(); }
		 "/// <summary>".Dump();
		$"/// {desc}".Dump();
		"/// </summary>".Dump();
		$"public bool? {name} {{ get; set; }}".Dump();
	}
}
